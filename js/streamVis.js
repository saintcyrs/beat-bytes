...
class streamVis {
    ...
    async initVis() {
        // Load data
        const data = await d3.csv(this.dataFile);
        this.data = data;
    ...
    updateVis() {
        ...
        const visibleData = this.data.slice(this.currentIndex, this.currentIndex + this.maxVisibleBoxes);
        const boxContainer = d3.select(this.element);
        const boxes = boxContainer.selectAll('.data-box')
        ...
        boxes.enter()
            ...
            .style('left', '0px')
            .text(d => `Track: ${d.track_name}, Streams: ${Number(d.streams).toLocaleString()}`)
            .transition()
        ...
        boxes.text(d => `Track: ${d.track_name}, Streams: ${Number(d.streams).toLocaleString()}`);
    }
    ...
    startAnimation() {
        ...
        d3.selectAll('.data-box').each(function() {
            ...
            const remainingDuration = (remainingDistance / window.innerWidth) * boxMoveDuration;
